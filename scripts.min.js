...
function moveLeft(slides, sWidth, sLength, sShow) {
    sLength - sShow > counter && (slides.animate({
        left: "-=" + sWidth + "px"
    }, 300, function() {}), counter += 1);
}

function moveRight(slides, sWidth) {
    counter > 0 && (slides.animate({
        left: "+=" + sWidth + "px"
    }, 300, function() {}), counter -= 1);
}
    ...
    $.fn.lazyload = function(options) {
        function update() {
            ...
            elements.each(function() {
                ...
                } else $this.trigger("appear"), /* if we found an image we'll load, reset the counter */
                counter = 0;
            });
        }
        ...
        return options && (undefined !== options.failurelimit && (options.failure_limit = options.failurelimit, 
        delete options.failurelimit), undefined !== options.effectspeed && (options.effect_speed = options.effectspeed, 
        delete options.effectspeed), $.extend(settings, options)), $container = settings.container === undefined || settings.container === window ? $window : $(settings.container), 
        0 === settings.event.indexOf("scroll") && $container.bind(settings.event, function() {
            return update();
        }), this.each(function() {
            var self = this, $self = $(self);
            self.loaded = !1, ($self.attr("src") === undefined || $self.attr("src") === !1) && $self.is("img") && $self.attr("src", settings.placeholder), 
...
var sLength = jQuery(".slide").length, sWidth = jQuery(".slide").outerWidth(!0), wrapperWidth = sWidth * (sLength + 1), sShow = 2;
...
jQuery(".left").on("click", function(e) {
    e.preventDefault(), moveLeft(jQuery(".slide-wrapper"), sWidth, sLength, sShow), 
    console.log(counter);
}), jQuery(".right").on("click", function(e) {
    e.preventDefault(), moveRight(jQuery(".slide-wrapper"), sWidth, sLength), console.log(counter);
}), $(document).ready(function() {
    //set height of #home section and its child divs
    var winHeight = $(window).height(), double = 2 * winHeight;
    $("#home").height(double), $("iframe").height(winHeight), $(".one").height(winHeight);
    //place playvideo button in center of screen
    var btnHeight = $(".video-btn img").height(), center = winHeight / 2 - btnHeight / 2;
    $(".video-btn").css("bottom", center);
    //set parallax scrolling on £home section
    var obj = $("#home");
    $(window).scroll(function() {
        var $window = $(this), yPos = -($window.scrollTop() / obj.data("speed")), coords = "50% " + yPos + "px";
    ...
    $("a.video-btn").on("click", function(e) {
        e.preventDefault(), $(".video").append(vid);
    });
...
var modal = function() {
    var $window = $(window), $modal = $('<div class="modal"/>'), $content = $('<div class="modal-content"/>'), $close = $('<button role="button" class="modal-close">close</button>');
    return $modal.append($content, $close), $close.on("click", function(e) {
        ...
        open: function(settings) {
            $content.empty().append(settings.content), $modal.css({
                width: settings.width || "auto",
                height: settings.height || "auto",
                opacity: 1
            }).appendTo("body"), modal.center(), $window.on("resize", modal.center);
        },
        close: function() {
            $content.empty(), $window.off("resize", modal.center);
        }
    };
}();
...
var Navbar = function(btn, nav, fixedState) {
    function highlighter(hash, top, viewportTop) {
        viewportTop > top - 200 && $(".navigation ul li a").each(function(index) {
            this.hash == hash ? $(this).addClass("selected") : $(this).removeClass("selected");
        });
    }
    this.btn = btn, this.nav = nav, this.toggleNav = function() {
        btn.on("click", function(e) {
            e.preventDefault(), nav.parent().toggle();
        });
    }, this.scrollToDiv = function() {
        nav.on("click", "a", function(e) {
            e.preventDefault();
            var hash = $(this).attr("href"), pos = $(hash).offset();
            $("html, body").animate({
                scrollTop: pos.top
            }, "slow");
        });
        ...
        $(window).bind("load resize", function() {
            ...
            850 > winWidth ? (nav.parent().hide(), btn.fadeIn()) : (nav.parent().fadeIn(), btn.hide());
        });
    }, this.scrolly = function() {
        $(window).bind("scroll load", function() {
            for (var viewportTop = $(window).scrollTop(), a = ($(document).innerHeight(), $(".navigation ul li a")), i = 0; i < a.length; i++) {
                var currentAnchor = a[i], nextAnchor = a[i], hash = $(currentAnchor).attr("href"), top = ($(nextAnchor).attr("href"), 
                $(hash).offset().top);
...
var navi = function() {
    var $window = $(window), $navUl = $(".navigation ul");
    return {
        checkWidth: function() {
            return $window.width();
        },
        toggle: function() {
            (navi.checkWidth() < 750 || $window.on("resize", navi.checkWidth) < 750) && ($navUl.hide(), 
            $(".responsive-menu-btn").on("click", function(e) {
                e.preventDefault(), $navUl.toggle();
            }));
        },
...
!function() {
    navi.toggle(), navi.scrollToDiv(), navi.scrollNav();
    ...
    a.fn.viewportChecker = function(b) {
        ...
        return this.checkElements = function() {
            ...
            g = b + e.height), d.each(function() {
                ...
                a.extend(f, c), a.extend(f, h), !d.hasClass(f.classToAdd) || f.repeat) {
                    ...
                    f.invertBottomOffset && (j -= 2 * f.offset), g > i && j > b ? (d.removeClass(f.classToRemove), 
                    d.addClass(f.classToAdd), f.callbackFunction(d, "add")) : d.hasClass(f.classToAdd) && f.repeat && (d.removeClass(f.classToAdd), 
                    f.callbackFunction(d, "remove"));
                }
            });
        }, a(document).bind("touchmove MSPointerMove pointermove", this.checkElements), 
        a(window).bind("load scroll touchmove", this.checkElements), a(window).resize(function(b) {
            e = {
                height: a(window).height(),
                width: a(window).width()
            }, d.checkElements();
